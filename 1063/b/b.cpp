...
void bfs(){
    ...
    vis[r][c]=1;
    while(!q.empty()){
        int nx=q.front().ax,ny=q.front().ay,a=q.front().a,b=q.front().b;q.pop_front();
        ans++;
        if(!vis[nx+1][ny]&&nx<n){
            q.push_front((node){nx+1,ny,a,b});
            vis[nx+1][ny]=1;
        ...
        if(!vis[nx][ny+1]&&ny<m&&b){
            q.push_back((node){nx,ny+1,a,b-1});
            vis[nx][ny+1]=1;

        }
        if(!vis[nx][ny-1]&&ny>1&&a){
            q.push_back((node){nx,ny-1,a-1,b});
            vis[nx][ny-1]=1;
