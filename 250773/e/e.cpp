...
int main(){
    ...
    for(int i = n; i > 0; i--)
        for(int j = 1; j <= m; j++){
            auto ia = lower_bound(a.begin(), a.end(), i+j);
            auto ib = lower_bound(b.begin(), b.end(), i+m+1-j);
            if(ia != a.end()) a.erase(ia);
            else if(ib != b.end()) b.erase(ib);
            else{
                cout<<"NO"<<endl;
                return 0;
            }
