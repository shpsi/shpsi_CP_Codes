...
int kmp(string &t, string &s) {
    ...
    for(int i = 1; i < s.size(); i++) {
        while(cur > 0 && s[cur] != s[i]) cur = f[cur];
        if (s[cur] == s[i]) cur++;
        f[i + 1] = cur;
    ...
    cur = 0;
    for(int i = max(t.size() - s.size(), 0); i < t.size(); i++) {
        while(cur > 0 && s[cur] != t[i]) cur = f[cur];
        if (s[cur] == t[i]) cur++;
        if (cur == s.size() && i == t.size() - 1) return s.size();
        if (cur == s.size()) cur = f[cur];
